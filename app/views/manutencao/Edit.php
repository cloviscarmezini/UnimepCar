        ...
        <div class="col-12">
            <form action="<?= URL_BASE.'manutencao/edit'?>" method="POST">
                                ...
                                <?php
                                    if(is_array($veiculos)){
                                        foreach($veiculos as $v){
                                            if($v->id == $manutencao['veiculo_id']){
                                                echo '<option value="'.$v->id.'" selected>'.$v->fabricante.'  | '.$v->modelo.'</option>';
                                            }
                                            else{
                                                echo '<option value="'.$v->id.'">'.$v->fabricante.'  | '.$v->modelo.'</option>';
                                            }
                                        }
                                    }
                                    else{
                                        echo '<option value="'.$veiculos->id.'" selected>'.$veiculos->fabricante.'  | '.$veiculos->modelo.'</option>';
                                    }
                                ?>
                            </select>
                        </div>
                ...
                <fieldset class="form-group border rounded p-5 itens">
                    ...
                    <?php if($itensManutencao){foreach($itensManutencao as $i){?>
                        <div class="form-row item border-top py-3">
                            <div class="form-group col-md-6">
                                <label for="desc_item">Item adicionado</label>
                            ...
                            <div class="form-group col-md-2 text-center">
                                <label>Remover item</label><a class="text-danger h3 d-block remove_item"><i class="fas fa-minus-circle"></i></a>
                            </div>
                        </div>
                    <?php }} ?>
                </fieldset>
                        ...
                        <?php foreach($pecaManutencao as $idx=>$pm){?>
                            <div class="form-row">
                                ...
                                <div class="form-group col-md-2">
                                    ...
                                    <input class="form-control" name="qtd_peca[]" value="<?= $pm->quantidade?>">
                                </div>
                                <div class="form-group col-md-2">
                                    <label>Estoque Atual</label>
                                    <input class="form-control" value="<?= $pecas[$idx]->estoque?>" readonly>
                                    <input name="peca[]" type="hidden" value="'+peca+'">
                                </div>
                                <div class="form-group col-md-2 text-center">
                                    <label>Remover peça</label>
                                    <a class="text-danger h3 d-block remove_item">
